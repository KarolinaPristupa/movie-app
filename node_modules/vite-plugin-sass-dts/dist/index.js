...
var parseCss = async (file, fileName, config) => {
  ...
  if (options.api === "legacy") {
    ...
    if (options.importers) {
      const userImporters = Array.isArray(options.importers) ? options.importers : [options.importers];
      for (const importer of userImporters) {
        if ("findFileUrl" in importer && typeof importer.findFileUrl === "function") {
          const legacyImporter = importer;
          finalImporters.push({
            canonicalize: async (url, context) => {
              const result2 = await legacyImporter.findFileUrl(url, {
            ...
            load: async (canonicalUrl) => {
              ...
              if (fs.existsSync(filePath) && fs.statSync(filePath).isDirectory()) {
                ...
                for (const indexFile of indexFiles) {
                  const indexPath = path2.join(filePath, indexFile);
    ...
    const internalModernImporter = {
      ...
      load: async (canonicalUrl) => {
        ...
        const contents = result2 && "contents" in result2 ? result2.contents : fs.readFileSync(
          result2 && "file" in result2 ? result2.file : filePath,
          "utf-8"
        );
...
var writeToFile = async (prettierOptions, fileName, classNameKeys, options) => {
  ...
  const exportName = getReplacerResult(baseName, options?.exportName) ?? "classNames";
  let exportTypes = "";
  let namedExports = "";
  const exportStyle = options?.esmExport ? `export default ${exportName};` : `export = ${exportName};`;
...
var getReplacerResult = (fileName, replacer) => {
    ...
    fs2.readFile(fileName, async (err, file) => {
      if (err) {
        ...
        try {
          ...
          if (!!css.globalStyle && option.global?.generate && option.global?.outputFilePath) {
...
// src/index.ts
var { resolveConfig, resolveConfigFile } = prettier2;
function Plugin(option = {}) {
  ...
  return {
    ...
    async configResolved(config) {
      ...
      const configPath = option.prettierFilePath ? await resolveConfigFile(option.prettierFilePath) : null;
      const prettierOptions = await resolveConfig(configPath || config.root, {
        config: configPath || void 0
      }) || {};
      cacheConfig = {
        ...config,
